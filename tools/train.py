...
from okeydrive.runtime_privacy import install_redacting_excepthook, redact_local_paths
...
def main():
    ...
    # Persist and log a redacted snapshot while retaining runtime values in cfg.
    sanitized_config = redact_local_paths(cfg.pretty_text)
    with open(osp.join(cfg.work_dir, osp.basename(args.config)), "w", encoding="utf-8") as stream:
        stream.write(sanitized_config)
