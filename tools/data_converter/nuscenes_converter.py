...
def create_nuscenes_infos(root_path,
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        ...
        only_scenes=val_scenes if only_val and not test else None)

    metadata = dict(version=version)
    if only_val and not test:
...
def get_available_scenes(nusc):
    ...
    Returns:
        available_scenes (list[dict]): List of basic information for the
            available scenes.
    """
    # The complete nuScenes release is present locally.  On a Windows-mounted
    # drive, checking one lidar file for every scene is disproportionately
    # slow; allow the caller to skip those existence probes.
    if os.getenv('OKEYDRIVE_ASSUME_FULL_NUSCENE', '0') == '1':
...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        if only_scenes is not None and sample['scene_token'] not in only_scenes:
            continue
...
def get_ego_status(nusc, nusc_can_bus, sample):
    ...
    try:
        cache_key = (id(nusc_can_bus), ref_scene['name'])
        if cache_key not in _CAN_BUS_MESSAGE_CACHE:
            _CAN_BUS_MESSAGE_CACHE[cache_key] = (
                nusc_can_bus.get_messages(ref_scene['name'], 'pose'),
                nusc_can_bus.get_messages(ref_scene['name'], 'steeranglefeedback'),
            )
        pose_msgs, steer_msgs = _CAN_BUS_MESSAGE_CACHE[cache_key]
...
parser.add_argument(
    '--only-val', action='store_true',
    help='generate only the trainval validation info file')
