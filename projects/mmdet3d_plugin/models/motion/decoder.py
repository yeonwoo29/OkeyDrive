...
class HierarchicalPlanningDecoder(object):
    ...
    def decode(
        ...
        for i, (cls, pred) in enumerate(zip(classification, prediction)):
            ...
            # The queue is useful for debugging but makes a full validation
            # pickle several gigabytes and can leave an incomplete pickle on
            # shared Windows/WSL drives. Keep the original output by default;
            # compact mode is used for metric-only experiments.
            if os.getenv("OKEYDRIVE_COMPACT_RESULTS", "0") != "1":
                result.update(
                    {
