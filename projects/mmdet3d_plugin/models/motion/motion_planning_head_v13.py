...
class V13MotionPlanningHead(BaseModule):
    def __init__(
        ...
        self.okeydrive_refiner = (
            ContextMotionMambaRefiner(**okeydrive_refiner)
            if okeydrive_refiner is not None
            else None
        )

    def _run_okeydrive_refiner(
        self,
        plan_anchor,
        plan_query,
        ...
        """Run the candidate-sequence Mamba path without diffusion timesteps."""

        if self.okeydrive_refiner is None:
            raise RuntimeError("the OkeyDrive refiner is not configured")
        context = metas.get("okeydrive_context")
        if context is None:
            raise RuntimeError("OkeyDrive planning requires the integrated feature context")
        batch = plan_anchor.shape[0]
        navigation_features = plan_query.squeeze(1).reshape(
            batch, plan_anchor.shape[1], self.ego_fut_mode, self.embed_dims
        )
        sparse_context = 0.5 * (
            instance_feature_selected.mean(dim=1)
            + map_instance_feature_selected.mean(dim=1)
        )
        return self.okeydrive_refiner(
            base_trajectories=plan_anchor,
            navigation_features=navigation_features,
            sparse_context=sparse_context,
            bev=context["bev"],
            object_boxes=context["object_boxes"],
            object_visibility=context["object_visibility"],
            object_class=context["object_class"],
            object_valid=context["object_valid"],
        )
    ...
    def forward_train(
        ...
        if self.okeydrive_refiner is not None:
            okeydrive_output = self._run_okeydrive_refiner(
                plan_anchor,
                plan_query,
                instance_feature_selected,
                map_instance_feature_selected,
            ...
            candidate_count = okeydrive_output["prediction"].shape[2]
            planning_output["diffusion_prediction"] = [
                okeydrive_output["prediction"].reshape(
                    bs, 1, 3 * candidate_count, self.ego_fut_ts, 2
                )
            ]
            planning_output["diffusion_classification"] = [
                okeydrive_output["score"].reshape(bs, 1, 3 * candidate_count)
            ]
            batch_indices = torch.arange(bs, device=device)
            command = metas["gt_ego_fut_cmd"].argmax(dim=-1)
            planning_output["tgt_cmd_plan_anchor"] = okeydrive_output["base_increment"][
                batch_indices, command
            ]
            planning_output["okeydrive"] = okeydrive_output
    ...
    def forward_test(
        ...
        if self.okeydrive_refiner is not None:
            ...
            candidate_count = okeydrive_output["prediction"].shape[2]
            planning_output["prediction"] = [
                okeydrive_output["prediction"].reshape(
                    bs, 1, 3 * candidate_count, self.ego_fut_ts, 2
                )
            ]
            planning_output["classification"] = [
                okeydrive_output["score"].reshape(bs, 1, 3 * candidate_count)
            ]
            planning_output["okeydrive"] = okeydrive_output
            return motion_output, planning_output
