    ...
    def get_ann_info(self, index):
        ...
        if 'gt_ego_fut_trajs' in info:
            ...
            for i in range(1, fut_ts + 1):
                # The official loop assumes that every sample has all future
                # entries in the validation pickle.  Guard the final sample
                # so truncated/smoke files (and a scene ending at val) do not
                # raise IndexError before the scene-token check can run.
